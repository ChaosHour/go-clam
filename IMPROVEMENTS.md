...
**#8 streaming half (follow-up branch):** clamd mode now streams discovery directly into the workers via an emit-callback `discoverFiles` — scanning starts immediately, the file list is never materialized, the progress display runs as a counter until "Discovery complete: N files" arrives mid-scan, and the skip breakdown prints with it. clamscan batch mode still collects the list upfront because `--file-list` chunks must be complete before launch. With this, every item in the review is implemented.
